...
const USER_DATA = [
  { id: "u1", name: "Đỗ Tiểu Tuấn Thị", email: "dogiang2902@gmail.com", phone: "0862183019" },
  { id: "u2", name: "Nguyễn Văn A", email: "vana@company.com", phone: "0901234567" },
];
...
const UserTable = () => (
  <View style={styles.tableWrapper}>
    ...
    <View style={[styles.row, styles.header]}>
      <Text style={[styles.cell, styles.headerText, { flex: 2 }]}>Tên</Text>
      <Text style={[styles.cell, styles.headerText, { flex: 2 }]}>Email</Text>
      <Text style={[styles.cell, styles.headerText, { flex: 1 }]}>Điện thoại</Text>
    </View>
    ...
    {USER_DATA.map((user) => (
      <View key={user.id} style={styles.row}>
        ...
        <Text style={[styles.cell, { flex: 1 }]}>{user.phone}</Text>
      </View>
    ))}
  </View>
);
...
const EMPLOYEE_DATA = [
  { id: "e1", name: "Phạm Văn C", position: "Nhân viên kinh doanh", status: "Đang hoạt động" },
  { id: "e2", name: "Lê Thị B", position: "Kế toán", status: "Tạm dừng" },
];
const EmployeeTable = () => (
  <View style={styles.tableWrapper}>
    {/* Header */}
    <View style={[styles.row, styles.header]}>
      <Text style={[styles.cell, styles.headerText, { flex: 2 }]}>Tên</Text>
      <Text style={[styles.cell, styles.headerText, { flex: 2 }]}>Chức vụ</Text>
      <Text style={[styles.cell, styles.headerText, { flex: 1 }]}>Trạng thái</Text>
    </View>

    {/* Body */}
    {EMPLOYEE_DATA.map((emp) => (
      <View key={emp.id} style={styles.row}>
        <Text style={[styles.cell, { flex: 2 }]}>{emp.name}</Text>
        <Text style={[styles.cell, { flex: 2 }]}>{emp.position}</Text>
        <Text style={[styles.cell, { flex: 1 }]}>{emp.status}</Text>
      </View>
    ))}
  </View>
);
                ...
                <View style={styles.tabBar}>
                    ...
                    <View style={styles.tabGroup}>
                            ...
                            return (
                                ...
                                onPress={() => setActiveTab(tab as "Người dùng" | "Nhân viên")}
                                >
...
{/* Nội dung bảng */}
      <ScrollView style={styles.content}>
        {activeTab === "Người dùng" ? <UserTable /> : <EmployeeTable />}
      </ScrollView>
            {/* Footer Phân trang */}
...
const styles = StyleSheet.create({
    tableWrapper: { borderWidth: 1, borderColor: "#ddd", borderRadius: 6, overflow: "hidden" },
  row: { flexDirection: "row", borderBottomWidth: 1, borderBottomColor: "#ddd" },
  header: { backgroundColor: "#f5f5f5" },
  cell: { padding: 10, fontSize: 14, color: "#333" },
